...
function jobs_list()
{
	...
	$sb = new sqlbuilder('mdt',"select * from ".TB_PREFIX."jobs where lastTime>='".date("Y-m-d")."' and channelId=$params[id]",'ordering desc',$db,jobsCount);
	if($sb->results)
	{
		foreach($sb->results as $rst)
		{
			...
			{
				?>
    			<td colspan="2" align="right"><a href="/html/<?php echo $pfileName ?>/send_<?php echo $rst['id'] ?>.html"><strong style="color:Green;">应聘此职位</strong></a></td>
				<?php 
			}
			else
			{
