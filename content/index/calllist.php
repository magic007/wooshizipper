...
function dt_calllist($channelId,$n=0,$strcount=0,$stcountl=0,$style=0,$isellipsis=true)
{
	...
	{
		if($n == 0)
			$results= $db->get_results("select * from ".TB_PREFIX."list as a,".TB_PREFIX."list_content as b  where a.guid=b.guid and a.channelId in(".$navId.") order by a.dtTime desc");
		else
			$results= $db->get_results("select * from ".TB_PREFIX."list as a,".TB_PREFIX."list_content as b  where a.guid=b.guid and a.channelId in(".$navId.") order by a.dtTime desc limit $n");
		if($results)
		{
