<?php
class c_casus extends DtDatabase
{
	public $id;
	public $isTopic;
	public $isImportant;
	...
	{
		$this->table_name = TB_PREFIX.'casus';
		$this->DtDatabase();		
}
	public function get_request($request=array())
	{
		...
		if($request['calling'])$this->calling=$request['calling'];
	...
	public function save()
	{
		...
		$sql.=isset($this->calling)?"calling,":'';
...
if(substr($sql,strlen($str)-1,1)==',')$sql=substr($sql,0,strlen($str)-1);		$sql.=")VALUES (";
		...
		$sql.=isset($this->calling)?"'$this->calling',":'';
...
if(substr($sql,strlen($str)-1,1)==',')$sql=substr($sql,0,strlen($str)-1);		$sql.=')';
		...
		$sql.=isset($this->calling)?"`calling`='$this->calling',":'';
