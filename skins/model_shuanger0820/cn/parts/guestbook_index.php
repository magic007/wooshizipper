...
{
	?>
	<form id="form1" name="form1" method="post" action="/?p=<?php data('page.id')?>&a=create" onsubmit="return validator()">
	<?php
}
else
{
...
?>
  ...
  <tr>
    <td>留言内容：</td>
    <td colspan="3"><textarea name="content" style="width:80%;height:60px; font-size:12px;" onMouseOver="this.style.borderColor='#9ecc00'"           
...
function guestbook_list()
{
	...
	if(URLREWRITE)
	$sb = new sqlbuilder('mdt',"select * from ".TB_PREFIX."guestbook where channelId=$params[id] and isPublic=1",'id desc',$db,guestbookCount,true,'/');
	else
		$sb = new sqlbuilder('mdt',"select * from ".TB_PREFIX."guestbook where channelId=$params[id] and isPublic=1",'id desc',$db,guestbookCount,true,'./');
	if($sb->results)
	{
