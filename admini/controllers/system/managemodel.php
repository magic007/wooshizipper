...
function index()
{
	...
	$sb = new sqlbuilder('mdt','select * from `'.TB_PREFIX.'models_reg` where readonly=0','id',$db,20);
	...
	$model_list->add_col('操作','edit','text',140,'"<a href=\"./?m=system&s=managemodel&a=unstall_model&model_type=$rs[type]&l='.$request['l'].'\" onclick=\"return confirm(\'您确认要删除本模块?一旦删除，将不可恢复。\');\">[删除]</a>"');
}
function upload_model()
{
	...
	if(file_exists(ABSPATH.'/temp/'.$fileName))
	{
		...
		if(unzip(ABSPATH.UPLOADPATH.'temp/',ABSPATH.'/temp/'.$fileName,ABSPATH.'/temp/'.$fileName)==1)
		{
			...
			redirect('?m=system&s=managemodel&l='.$request['l']);
		}
	}
}
...
{
	global $db,$request;
	$uninstall = $db->get_row('select * from `'.TB_PREFIX."models_reg` where type='$request[model_type]'");
	if($uninstall)	
	{
		$doc = get_config_xmldoc('install');
