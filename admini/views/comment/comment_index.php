...
function comment_index()
{
	...
	if($request['s'] == 'newcomment')
	{
		...
		$comment_sb = new sqlbuilder('comment_mdt','select * from `'.TB_PREFIX.'comment` where auditing=0 and channelId='.$request['p'].' and recordId='.$request['n'],'id desc',$db,6,true,'./','#adminiCommentPosition');
	}
	else
	{
		...
		if(!empty($request['comment_mdtp'])&&$request['comment_mdtp'] != '1')
		$count = $count - (intval($request['comment_mdtp']) - 1)*6;
		$comment_sb = new sqlbuilder('comment_mdt','select * from `'.TB_PREFIX.'comment` where channelId='.$request['p'].' and recordId='.$request['n'],'id desc',$db,6,true,'./','#adminiCommentPosition');
	}
	?>
	<form name="commentForm" method="post">
	<table width="100%" border="0" cellpadding="2" cellspacing="1" bgcolor="#C5EAF5"> 
	  <tr>
		<td>该内容评论管理 | <?php echo $comment_sb->get_pager_show() ?></td>
		<td align="right"><input type="button" name="button1" value="全选" onclick="checkAll()" /><input type="button" name="button2" value="反选" onclick="deleteAll()" />&nbsp;&nbsp;<input type="button" name="button3" value="审核所选" onclick="commentForm.action='./?a=auditingAllComment&p=<?php echo $request['p'] ?>&n=<?php echo $request['n'] ?>';commentForm.submit();" /><input type="button" name="button4" value="删除所选" onclick="commentForm.action='./?a=destroyAllComment&p=<?php echo $request['p'] ?>&n=<?php echo $request['n'] ?>';commentForm.submit();" />&nbsp;&nbsp;</td>
	  </tr>
	  <tr>
		  ...
		  <?php
			if(isset($comment_sb->results))
			{
				...
				{			
					...
					?></td>
					<td style="width:130px;"><input type="checkbox" name="comment_operation_all[]" value="<?php echo $o['id'] ?>" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" name="bt33" value="审核" onclick="window.location='./?a=auditingComment&p=<?php echo $request['p'] ?>&n=<?php echo $request['n'] ?>&comment=<?php echo $o['id'] ?>'" /><input type="button" name="bt32" value="删除" onclick="window.location='./?a=destroyComment&p=<?php echo $request['p'] ?>&n=<?php echo $request['n'] ?>&comment=<?php echo $o['id'] ?>'" /></td>
					</tr>
					<tr>
					  ...
					  <td style="width:130px;height:25px;<?php if ($count%2 ==0) {?> background-color:#D0FDF4;<?php } else {?> background-color:#E3E3E3;<?php }?>"><a href="<?php echo $o['homepage'] ?>">浏览主页</a>&nbsp;|&nbsp;<a href="mailto:<?php echo $o['email'] ?>">回复邮件</a></td>
					</tr>
		  			</table>					
					<?php
					$count--;
