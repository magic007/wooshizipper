<?php require(ABSPATH.'/admini/views/system/bakup/header.php') ?>
 <table width="100%">
 <form method="post" name="myform" action="?m=system&amp;s=bakup&a=export&l=<?php echo $request['l']?>">
  <tr align="center">
...
?>
  <tr bgcolor="<?php echo $info['bgcolor']?>"  align="center">
    ...
    <td><?=$id?></td>
    <td class="px10" align="left">&nbsp;<?php echo $info['filename']?></td>
    <td class="px10"><?php echo $info['filesize']?> K</td>
	<td class="px10"><?php echo $info['maketime']?></td>
    <td class="px10"><?php echo $info['number']?></td>
    <td>
	<a href="?m=system&amp;s=bakup&a=import&pre=<?php echo $info['pre']?>&dosubmit=1&l=<?php echo $request['l']?>">导入</a> | 
	<a href="?m=system&amp;s=bakup&a=delete&filenames=<?php echo $info['filename']?>&l=<?php echo $request['l']?>" onclick="return confirm('您确认要删除此数据库备份?一旦删除，将不可恢复。')">删除</a> | 
	<a href="?m=system&amp;s=bakup&a=download&filename=<?php echo $info['filename']?>&l=<?php echo $request['l']?>">下载</a>
	</td>
</tr>
<?php 
	}
...
?>
  ...
  <tr>
	...
	<td colspan=6 valign="top" class="tablerow">全选/反选 <input type="submit" name="submit2" value=" 删除选中的备份 " onclick="document.myform.action='?m=system&amp;s=bakup&a=delete&l=<?php echo $request['l']?>'"></td>
  </tr>
	</form>
</table>
<P></P>
<table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#C5EAF5">
	<form name="upload" method="post" action="?m=system&amp;s=bakup&a=uploadsql&l=<?php echo $request['l']?>" enctype="multipart/form-data">
  <tr>
  ...
  <tr>
	<td colspan=2 valign="top" class="tablerow">&nbsp;<span style="color:red">注意:</span>上传文件格式须为SQL格式，且命名格式为：数据库名称+下划线_+8位备份日期+下划线_+6位随机数+数据表备份卷号，如deepthroat_20080722_247512_1.sql。</td>
  </tr>
  </form>
</table>